...
namespace MyUtilities
{
    ...
    class MyVar
    {
        ...
        {
            if (s == null) return StringtoInt(defaultval);
            try
            {
#if DEBUG
        ...
        /// <summary>
        /// Verify string is an HexDecimal Format, Return True if integer</summary>
        public bool VerifyHexStringInString(string hexString)
        {
            if (hexString != null || hexString != "")
            {
                hexString = ByteArrayToHexString(HexStringToByteArray(hexString));
        ...
        public bool TryParseIpAddress(string input, out string baseIpAddress)
        {
            ...
            if (octets.Length == 4)
            {
                ...
                if (int.TryParse(octets[0], out var octet1) &&
                    ...
                    int.TryParse(octets[3], out var octet4))
                {
                    // Check octet ranges
                    if (octet1 >= 0 && octet1 <= 255 &&
                        ...
                        octet4 >= 0 && octet4 <= 255)
                    {
                        // Combine the four octets
                        baseIpAddress = $"{octet1}.{octet2}.{octet3}.{octet4}";
                        return true;
                    }
                }
            }
        ...
        /// <summary> 
        /// Network determine if a string ipaddress is valid</summary>
        public bool TryParseIpAddress3Octet(string input, out string baseIpAddress)
        {
            // Split the input into octets
            var octets = input.Split('.');

            // Ensure there are exactly four
            if (octets.Length == 4)
            {
                // Attempt to parse each octet
                if (int.TryParse(octets[0], out var octet1) &&
        ...
        /// <summary> 
        /// Evaluate IP Address string and remove invalid characters</summary>
        public string RemoveInvalidIPAddrChars(string text)
        {
